<!DOCTYPE html>
<html>

<head>
    <title>livre-or</title>
    <meta sharset="utf-8">
    <link rel="stylesheet" href="profil.css">
    </head>

        
<body>
    <header>
        <nav>
            <ul>
                <li><a href="index.php"><img id="cr"src="coeur.webp">P.A</a></li>
                <li><a href="connexion.php">CONNEXION</a></li>
                <li id="bas"><a href="inscription.php">INSCRIPTION</a></li>
                <li><a href="profil.php">PROFIL</a></li>
                <li id="bas"><a href="livre-or.php">LIVRE D'OR</a></li>
                <li><a href="commentaire.php">COMMENTAIRES</a></li>
            </ul>
        </nav>
    </header>

    <main>

        <section>
            <article id="align">  
                <p><strong> Modifiez votre profil sur</p><p id="nom">"PROTECT ANIMAL"</strong></p>
            </article>
        </section>

        <section>
            <article id="form-img">
                <form id="form1" method ="POST">
                    <label> Ancien Login</label><br>
                    <input type ="text" name ="login" placeholder ="Ancien login"/><br><br>
                    <label>nouveau Login</label><br>
                    <input type ="text" name ="login" placeholder ="Nouveau login"/><br><br>
                    <label>Ancien Password</label><br>
                    <input type="passeword" name="password" placeholder=" Ancien password"/><br><br>
                    <label>Nouveau Password</label><br>
                    <input type="passeword" name="password" placeholder=" Nouveau password"/><br><br>
                    <input type = "submit" value="Valider" name="submit"/>
                </form> 
            </article>
        </section>

    </main>


        <footer id="logo">

            <article>
                <p>Copyright 2019-2020 | All by Rights Reserved.</p>
            </article>

                <article>
                    <img height="60" src="footeror.webp">
                </article>

            <article>
                <p>Sieges social Rue Montreuil PARIS 75014<br> Siren 648328822774 | Siret 6483874678. </p>
            </article>
    
        </footer>

</body>
</html>